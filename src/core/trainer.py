...
from ..models.losses.semantic_center_loss import central_bce_loss, cmshc_full_loss
...
def _plot_loss_panels(history, keys_labels, dest: Path, nrows: int = 2) -> None:
    """Plot one matplotlib subplot per ``(key, label)`` pair in *keys_labels*.

    ``history`` is a list of per-epoch dicts (as written to
    ``loss_history.json``).  Missing keys default to ``0.0`` so partial logs
    still produce a usable figure.  Unused panels (when ``nrows * ncols``
    exceeds ``len(keys_labels)``) are hidden.
    """
    import matplotlib
    ...
    n = len(keys_labels)
    ncols = (n + nrows - 1) // max(nrows, 1)
    fig, axes = plt.subplots(
        nrows, ncols, figsize=(4.5 * ncols, 4 * nrows), tight_layout=True
    )
    axes_flat = list(axes.flat) if hasattr(axes, "flat") else [axes]

    for ax, (key, label) in zip(axes_flat, keys_labels):
...
def _plot_cmshc_losses(history, dest: Path) -> None:
    _plot_loss_panels(
        ...
        [
            ("center", "Central BCE"),
            ("quant", "Quantization (log cosh)"),
            ("cross_modal", "Cross-modal MSE"),
...
def _plot_dcmh_anchored_losses(history, dest: Path) -> None:
    _plot_loss_panels(
        history,
        [
            ("total_scaled", "Total (scaled)"),
            ("log_loss", "Log-likelihood (pair)"),
            ("quant_loss", "Quantization"),
            ("balance_loss", "Balance"),
            ("center_loss", "Center BCE (anchor)"),
            ("full_objective", "Full objective (DCMH)"),
        ],
        dest,
        nrows=2,
    )
...
# =============================================================================
# Anchored DCMH trainer (diagnostic)
# =============================================================================


class DCMHAnchoredTrainer(DCMHTrainer):
    """DCMH with an added CM-SHC-style center-BCE anchor term.

    Diagnostic trainer used to test whether hash-center supervision is
    the mechanism that lets CM-SHC absorb LoRA adapters while vanilla
    DCMH regresses on the same backbone.  The DCMH pair / quant /
    balance losses are unchanged; a per-sample ``central_bce_loss``
    against the pre-computed target codes ``T_train`` (from Stage 2 of
    CM-SHC) is added to both the image-update and text-update steps,
    weighted by ``lambda_center``.

    Compared with :class:`DCMHTrainer`, this class:

    * Requires a ``target_codes`` tensor of shape
      ``(num_train, bit_dim)`` with values in ``{-1, +1}``.
    * Tracks a new ``center_loss`` metric per epoch (logged to
      ``loss_history.json`` and plotted alongside the DCMH terms).
    * Saves the anchor weight to the checkpoint meta so downstream
      analysis knows which λ was used.

    The architecture is identical to DCMH -- only the loss changes --
    so ``build_model`` continues to instantiate a plain ``DCMH`` module.
    """
    ...
    def train_epoch(self):
        ...
        for batch in tqdm(self.train_loader, desc="img", leave=False):
            ind_np = (
                ...
                else np.asarray(batch["index"])
            )
            ind_t = torch.as_tensor(ind_np, dtype=torch.long, device=self.device)
            image = batch["img"].to(self.device)
            sample_L = batch["label"].to(self.device).float()
            if sample_L.dim() == 1:
                sample_L = torch.nn.functional.one_hot(
            ...
            cur_f = self.model.encode_image(image)
            with torch.no_grad():
                self.F_buffer[ind_np, :] = cur_f.detach()

            logloss, quant, bal = dcmh_batch_loss_image(
                cur_f, sample_L, self.train_L,
                self.G_buffer, self.F_buffer, self.B,
                ind_np, self.ones, self.ones_, self.num_train,
            )
            nll_scaled = logloss / (self.num_train * self.batch_size)
            quant_scaled = self.gamma * quant / (self.batch_size * self.bit)
            bal_scaled = self.eta * bal / (self.num_train * self.bit)
            ...
            loss = dcmh_loss + self.lambda_center * anchor
            ...
            metrics["quant_loss"] += float(quant.detach())
            metrics["balance_loss"] += float(bal.detach())
            metrics["center_loss"] += float(anchor.detach())
            metrics["total_scaled"] += float(loss.detach())
            metrics["n_img_steps"] += 1
        ...
        for batch in tqdm(self.train_loader, desc="txt", leave=False):
            ...
            if getattr(self.model, "text_backend", "transformer") == "mlp":
                ...
                cur_g = self.model.encode_text(input_ids, attn)
            with torch.no_grad():
                self.G_buffer[ind_np, :] = cur_g.detach()
        ...
        self.refresh_binary_codes()
        n = max(metrics["n_img_steps"] + metrics["n_txt_steps"], 1)
        for k in ("log_loss", "quant_loss", "balance_loss", "center_loss", "total_scaled"):
            metrics[k] /= n
        return metrics
    ...
    def train(
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            print(
                f"epoch {epoch + 1}/{self.max_epoch}  "
                f"log={m['log_loss']:.4f} quant={m['quant_loss']:.4f} "
                f"bal={m['balance_loss']:.4f} center={m['center_loss']:.4f}  "
                f"full_obj={full:.4f}"
            )
            ...
            if checkpoint_dir is not None:
                history_path.write_text(json.dumps(history, indent=2), encoding="utf-8")
                try:
                    _plot_dcmh_anchored_losses(history, checkpoint_dir / "loss_curve.png")
                except Exception:
                    pass
